    ...
    const fetchTests = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Filter only subject-wise tests
                const subjectWiseTests = data.filter(test => test.testType === 'subject-wise');
                setTests(subjectWiseTests);
            }
        } catch (error) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-100">
                <div>
                    <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Subject-Wise Tests</h2>
                    <p className="text-sm text-gray-600 mt-1">Manage mock tests organized by subjects (Math, History, Geography, etc.)</p>
                </div>
                <button
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 gap-4">
                    {tests.length === 0 ? (
                        <div className="text-center py-12 text-gray-500 bg-white rounded-xl border border-dashed">No Subject-Wise tests found. Create one to get started.</div>
                    ) : (
                        tests.map(test => (
                            <div key={test._id} className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                <div className="flex-1">
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-2">
                                        <span className="text-xs px-2 py-1 bg-green-100 text-green-800 rounded-full font-medium">Subject-Wise</span>
                                        {test.subject && <span className="text-xs px-2 py-1 bg-emerald-100 text-emerald-700 rounded-full font-medium">{test.subject}</span>}
...
export default SubjectWiseTestManager;
